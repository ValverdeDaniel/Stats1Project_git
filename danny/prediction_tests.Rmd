...
lasso_grid_search <- train(power.release ~ MONTHS_SINCE_BEG + total.inflows + total.release + evaporation + pool.elevation,
                       data = train_dat,
                       method = "lasso",
                       metric = "RMSE",
                       trControl = control)
...
predict(lasso_grid_search, newdata = test_dat)
test_dat$pool.elevation

predictions <- predict(lasso_grid_search, newdata = test_dat)
MSE_test <- sum((test_dat$pool.elevation - predictions)^2) / nrow(test_dat)
(RMSE <- sqrt(MSE_test))

## variable importance
varImp(lasso_grid_search)


print('predict')
# predict pool elevation at time i using last values for all other variables
i = 1200
predict(lasso_grid_search, newdata = data.frame(MONTHS_SINCE_BEG = i, 
                                            total.inflows = test_dat$total.inflows[nrow(test_dat)],
                                            total.release = test_dat$total.release[nrow(test_dat)],
                                            evaporation = test_dat$evaporation[nrow(test_dat)],
